...
export default function PerfilPet({ route, navigation }) {
  ...
  const deletarVacina = (idVacina, nomeVacina) => {
    Alert.alert(
      "Excluir Registro",
      `Deseja mesmo remover a aplicação de "${nomeVacina}" da carteirinha?`,
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              const vacinaDocRef = doc(db, "usuarios", uid, "pets", petSelecionado.id, "vacinas", idVacina);
              await deleteDoc(vacinaDocRef);

              setVacinas((prev) => prev.filter(v => v.id !== idVacina));
            } catch (error) {
              Alert.alert("Erro", "Não foi possível deletar o registro: " + error.message);
            }
          }
        }
      ]
          ...
          {loadingVacinas ? (
            ...
            vacinas.map((vacina) => (
              <View key={vacina.id} style={styles.vacinaItem}>
                ...
                <TouchableOpacity 
                  onPress={() => deletarVacina(vacina.id, vacina.nomeVacina)}
                  style={styles.deleteVacinaBtn}
                  activeOpacity={0.6}
                >
  ...
  sectionVacinas: { 
    backgroundColor: '#FFF',
     borderRadius: 24,
      ...
      borderWidth: 1,
       borderColor: '#E2E8F0', 
       ...Platform.select({ ios: { shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.06, shadowRadius: 12 }, android: { elevation: 4 } }) },
  vacinasHeader: { 
    flexDirection: 'row', 
    alignItems: 'center',
    gap: 8,
